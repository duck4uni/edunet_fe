...
export const useCourseDetail = () => {
  ...
  const transformedCourseData: Course = apiCourse ? {
    ...
    author: apiCourse.teacher ? `${apiCourse.teacher.firstName} ${apiCourse.teacher.lastName}` : 'Unknown',
    ...
    reviews: (Array.isArray(reviews) ? reviews : []).map(review => ({
      id: review.id,
      user: review.user ? `${review.user.firstName} ${review.user.lastName}` : 'Anonymous',
