// Admin Pages Export
export { default as AdminLogin } from './Login';
export { default as AdminForgotPassword } from './ForgotPassword';
export { default as AdminDashboard } from './Dashboard';
export { default as CourseManagement } from './Courses';
export { default as PendingCourseManagement } from './Courses/Pending';
export { default as RejectedCourseManagement } from './Courses/Rejected';
export { default as AdminCourseDetail } from './Courses/Detail';
export { default as TeacherManagement } from './Teachers';
export { default as EmployeeManagement } from './Employees';
export { default as RecruitmentManagement } from './Recruitment';
export { default as SupportManagement } from './Support';
export { default as PermissionManagement } from './Permissions';
export { default as RevenueManagement } from './Revenue';
export { default as ReviewManagement } from './Reviews';
export { default as AdminProfile } from './Profile';
export { default as TeacherRegistrationManagement } from './TeacherRegistrations';